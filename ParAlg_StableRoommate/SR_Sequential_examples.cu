		...
		for (int i = 0; i < N; i++)
		{
			...
			if (preference_lists[i].size() > 1)
			{
				...
				if (i == 1)
				{
					...
					for (int l = 0; l < preference_lists.size(); l++)
					{
						std::cout << l << " ";

						for (int m = 0; m < preference_lists[l].size(); m++)
						{
							std::cout << preference_lists[l][m] << " ";
						}
						std::cout << "\n";
					}
				}
...
int main()
{
	std::vector<std::string> input_names = { "inp.txt", "inp2.txt", "inp10.txt", "inp20.txt" };
